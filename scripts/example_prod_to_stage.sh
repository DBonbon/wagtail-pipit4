...
readonly REMOTE_MEDIA_PATH=/mnt/persist/www/leacards/shared/media
...
ssh $PROD_HOST "pg_dump -h localhost -Fc -f /tmp/db-dump.sql -U postgres leacards -x -O"
...
ssh $STAGE_HOST "sudo -u postgres pg_restore --clean -h localhost -d leacards -U postgres '/tmp/db-dump.sql'"
ssh $STAGE_HOST "rm /tmp/db-dump.sql"

rm /tmp/db-dump.sql

echo "Updating database..."
manage_prefix="source /mnt/persist/www/leacards/shared/venv/bin/activate && cd /mnt/persist/www/leacards/current/src &&"
